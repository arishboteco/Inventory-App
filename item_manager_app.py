...
if not db_engine:
    ...
    # --- Fetch Data (potentially including inactive) ---
    ...
    if items_df_with_stock.empty and 'name' not in items_df_with_stock.columns:
        ...
        st.dataframe(
            ...
            column_config={
                "item_id": st.column_config.NumberColumn("ID", width="small"), "name": st.column_config.TextColumn("Item Name", width="medium"),
                "unit": st.column_config.TextColumn("Unit", width="small"), "category": st.column_config.TextColumn("Category"), "sub_category": st.column_config.TextColumn("Sub-Category"),
                "permitted_departments": st.column_config.TextColumn("Permitted Depts", width="medium"), "reorder_point": st.column_config.NumberColumn("Reorder Lvl", width="small", format="%d"),
                "current_stock": st.column_config.NumberColumn("Current Stock", width="small", help="Calculated from transactions."), "notes": st.column_config.TextColumn("Notes", width="large"),
                "is_active": st.column_config.CheckboxColumn("Active?", width="small", disabled=True), # Display status
            },
    ...
    if not items_df_with_stock.empty and 'item_id' in items_df_with_stock.columns and 'name' in items_df_with_stock.columns:
        # Include (Inactive) marker in dropdown text if showing inactive items
    ...
    edit_options = [("--- Select ---", None)] + item_options_list # Use list that includes inactive marker if needed
    ...
    selected_item_tuple = st.selectbox( # Dropdown
        "Select Item to Edit / Deactivate / Reactivate:", options=edit_options, format_func=lambda x: x[0], key="item_to_edit_select", on_change=load_item_for_edit, index=current_index
    )
    if st.session_state.item_to_edit_id is not None and st.session_state.edit_form_values is not None:
        ...
        item_is_active = current_details.get('is_active', True) # Check status
        if item_is_active: # Show Edit/Deactivate for Active items
        ...
        else: # Show Reactivate for Inactive items
            ...
            if st.button("✅ Reactivate This Item", key="reactivate_button"):
                ...
                if reactivate_success:
                    st.success(f"Item '{current_details.get('name', '')}' reactivated successfully!"); get_all_items_with_stock.clear(); st.session_state.item_to_edit_id = None; st.session_state.edit_form_values = None; st.rerun()
    ...
    with st.expander("📈 Record Stock Adjustment"):
        ...
        with st.form("adjustment_form", clear_on_submit=True):
            ...
            if adj_submitted:
                ...
                elif math.isclose(adj_qty_change, 0): st.warning("Quantity Change cannot be zero.")
    ...
    with st.expander("📥 Record Goods Received"):
        # Filter item_options_list to only show ACTIVE items for receiving
        active_item_options_recv = [opt for opt in item_options_list if "(Inactive)" not in opt[0]]
        recv_item_options = [("--- Select Item Received ---", None)] + active_item_options_recv
    ...
    with st.expander("🗑️ Record Wastage / Spoilage"):
        # Filter item_options_list to only show ACTIVE items for wastage
        active_item_options_waste = [opt for opt in item_options_list if "(Inactive)" not in opt[0]]
        waste_item_options = [("--- Select Item Wasted ---", None)] + active_item_options_waste
