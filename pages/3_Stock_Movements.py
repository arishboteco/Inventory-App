...
st.write("Record stock coming in (Receiving), manual adjustments, or wastage.")
...
if not db_engine:
    ...
    items_df_stock_page = get_all_items_with_stock(db_engine, include_inactive=False) # Only active items
    ...
    with col1: # Receiving Column
        ...
        with st.expander("Enter Details", expanded=True): # Keep expanded maybe?
    ...
    with col2: # Adjustment Column
        ...
        with st.expander("Enter Details", expanded=True):
            # Allow adjusting active or inactive items - need full list if showing inactive on items page
            # For simplicity now, use active list, can change later if needed
