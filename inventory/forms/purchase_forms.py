...
class PurchaseOrderItemForm(StyledFormMixin, forms.ModelForm):
    ...
    def clean(self):
        ...
        if item and quantity and hasattr(item, 'minimum_order_qty') and item.minimum_order_qty:
            if quantity < item.minimum_order_qty:
                raise forms.ValidationError(
                    f"Quantity ({quantity}) is below minimum order quantity ({item.minimum_order_qty}) for this item"
                )
        
        # Price variance check - warn if price is significantly different from last price
        if item and unit_price and hasattr(item, 'last_purchase_price') and item.last_purchase_price:
            variance = abs(unit_price - item.last_purchase_price) / item.last_purchase_price
            if variance > 0.2:  # 20% variance threshold
                # This could be a warning rather than an error in a real implementation
                cleaned_data['_price_variance_warning'] = f"Price variance of {variance:.1%} from last purchase price"
