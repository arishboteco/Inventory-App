...
class NotificationManager {
    ...
    init() {
        ...
        if (!this.container) {
            ...
            this.container.className = 'fixed top-4 right-4 space-y-2 z-50 max-w-sm';
    ...
    showInline(target, message, type = 'info', duration = null) {
        ...
        const notification = this.createNotificationElement(message, type, 'inline');
    ...
    show(message, type = 'info', duration = null, options = {}) {
        ...
        const notification = this.createNotificationElement(message, type, style, options);
    ...
    createNotificationElement(message, type, style, options = {}) {
        ...
        const baseClasses = 'px-4 py-3 rounded-lg shadow-lg flex items-start gap-3 transition-all duration-300';
        ...
        if (options.dismissible !== false) {
            ...
            dismissButton.className = 'flex-shrink-0 text-xl leading-none opacity-70 hover:opacity-100';
            dismissButton.setAttribute('aria-label', 'Dismiss notification');
            dismissButton.onclick = () => this.dismiss(notification.dataset.notificationId);
    ...
    showBannerNotification(message, type, options) {
        ...
        const banner = this.createNotificationElement(message, type, 'banner', options);
...
window.showMessage = (message, type, duration) => window.notifications.showToast(message, type, duration);
window.showNotification = (message, type, duration) => window.notifications.showToast(message, type, duration);
