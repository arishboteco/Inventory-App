...
if not db_engine:
    ...
    # --- Tab 3: Edit/Manage Selected Supplier ---
    with tab_manage:
        st.subheader("Select Supplier to Manage")

        # Prepare supplier options for dropdown
        # Fetch fresh data within the tab context if needed, or ensure suppliers_df is available
        # Re-fetch here to ensure options reflect current state if Add/Deactivate happened
        suppliers_df_manage = get_all_suppliers(db_engine, include_inactive=st.session_state.show_inactive_suppliers)
        if not suppliers_df_manage.empty and 'supplier_id' in suppliers_df_manage.columns and 'name' in suppliers_df_manage.columns:
            supplier_options_list: List[Tuple[str, int]] = [
                (f"{row['name']}{'' if row['is_active'] else ' (Inactive)'}", row['supplier_id'])
                for index, row in suppliers_df_manage.dropna(subset=['name']).iterrows()
            ]
            supplier_options_list.sort()
        else: supplier_options_list = []
        edit_supplier_options = [("--- Select ---", None)] + supplier_options_list

        # Callback to load supplier details
        ...
        selected_supplier_tuple = st.selectbox(
            "Select Supplier:", options=edit_supplier_options,
        ...
        if st.session_state.supplier_to_edit_id is not None and st.session_state.edit_supplier_form_values is not None:
            ...
            if supplier_is_active: # Show Edit/Deactivate for Active suppliers
                ...
                # Deactivate Button (outside form)
            ...
            st.info("Select a supplier from the dropdown above to manage it.")
