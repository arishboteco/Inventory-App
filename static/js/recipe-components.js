...
(function() {
  ...
  function updateRowCost(row, options) {
    options = options || {};
    var scope = options.scope || row.closest('.drawer-panel') || row.closest('[data-modal-root]') || document;
    var quantityInput = row.querySelector('input[id$="-quantity"]');
    var quantity = quantityInput ? toNumber(quantityInput.value) : toNumber(row.dataset.quantity);
    row.dataset.quantity = String(quantity);

    var costPerBase = typeof options.costPerBaseUnit === 'number'
  ...
  function bindRowEvents(row, scope) {
    ...
    if (typeof window.initPredictiveDropdowns === 'function' && !row.dataset.predictiveInit) {
  ...
  function updateRecipeCosts(root) {
    ...
    rows.forEach(function(row) {
      ...
      if (row.dataset.hasZeroPrice === '1' || (hasItem && quantityValue > 0 && baseCostValue === 0)) {
    ...
    groups.forEach(function(group, idx) {
      ...
      headingCell.id = 'recipe-category-' + slugifyCategoryLabel(group.label) + '-' + idx;
