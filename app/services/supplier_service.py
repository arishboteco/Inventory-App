...
def get_all_suppliers(_engine: Engine, include_inactive=False) -> pd.DataFrame:
    ...
    """
    if _engine is None: 
        print("ERROR [supplier_service.get_all_suppliers]: Database engine not available.")
        return pd.DataFrame()
...
def get_supplier_details(engine: Engine, supplier_id: int) -> Optional[Dict[str, Any]]:
    ...
    """
    if engine is None: 
        print("ERROR [supplier_service.get_supplier_details]: Database engine not available.")
        return None
...
def add_supplier(engine: Engine, details: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    try:
        ...
        if new_id:
            ...
            return True, f"Supplier '{params['name']}' added successfully with ID {new_id}."
        else:
            return False, "Failed to add supplier (no ID returned)."
    except IntegrityError:
        return False, f"Supplier name '{params['name']}' already exists. Please use a unique name."
    except (SQLAlchemyError, Exception) as e:
        print(f"ERROR [supplier_service.add_supplier]: Database error adding supplier: {e}\n{traceback.format_exc()}")
        return False, "A database error occurred while adding the supplier."

def update_supplier(engine: Engine, supplier_id: int, updates: Dict[str, Any]) -> Tuple[bool, str]:
    """
    ...
    """
    if engine is None: return False, "Database engine not available."
    if not supplier_id or not updates: return False, "Invalid supplier ID or no updates provided."
    ...
    try:
        ...
        if result.rowcount > 0:
            ...
            return True, f"No changes detected for Supplier ID {supplier_id}. Update considered successful."
    except IntegrityError:
        return False, f"Update failed: Potential duplicate name '{updates.get('name')}'."
    except (SQLAlchemyError, Exception) as e:
        print(f"ERROR [supplier_service.update_supplier]: Database error updating supplier {supplier_id}: {e}\n{traceback.format_exc()}")
...
def deactivate_supplier(engine: Engine, supplier_id: int) -> Tuple[bool, str]:
    ...
    """
    if engine is None: return False, "Database engine not available."
    query = text("UPDATE suppliers SET is_active = FALSE, updated_at = NOW() WHERE supplier_id = :supplier_id;")
    try:
        with engine.connect() as connection:
            with connection.begin():
        ...
        print(f"ERROR [supplier_service.deactivate_supplier]: Error deactivating supplier {supplier_id}: {e}\n{traceback.format_exc()}")
...
def reactivate_supplier(engine: Engine, supplier_id: int) -> Tuple[bool, str]:
    ...
    """
    if engine is None: return False, "Database engine not available."
    query = text("UPDATE suppliers SET is_active = TRUE, updated_at = NOW() WHERE supplier_id = :supplier_id;")
    try:
        with engine.connect() as connection:
            with connection.begin():
                result = connection.execute(query, {"supplier_id": supplier_id})
        ...
        print(f"ERROR [supplier_service.reactivate_supplier]: Error reactivating supplier {supplier_id}: {e}\n{traceback.format_exc()}")
        return False, "A database error occurred."