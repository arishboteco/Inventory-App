...
def get_all_items_with_stock(_engine, include_inactive: bool = False) -> pd.DataFrame:
    ...
    items_query = text(items_query_sql); stock_query = text("""SELECT item_id, SUM(quantity_change) AS calculated_stock FROM stock_transactions GROUP BY item_id;""")
    ...
    try:
        with _engine.connect() as connection:
            items_df = pd.read_sql(items_query, connection); stock_levels_df = pd.read_sql(stock_query, connection)
            if items_df.empty: return pd.DataFrame(columns=expected_cols)
            items_df['item_id'] = items_df['item_id'].astype(int)
            if not stock_levels_df.empty: stock_levels_df['item_id'] = stock_levels_df['item_id'].astype(int); stock_levels_df['calculated_stock'] = pd.to_numeric(stock_levels_df['calculated_stock'], errors='coerce').fillna(0)
            else: stock_levels_df = pd.DataFrame(columns=['item_id', 'calculated_stock'])
            combined_df = pd.merge(items_df, stock_levels_df, on='item_id', how='left'); combined_df['calculated_stock'] = combined_df['calculated_stock'].fillna(0)
            ...
            for col in expected_cols:
                if col not in combined_df.columns: combined_df[col] = 0 if col in ['reorder_point', 'current_stock'] or col=='is_active' else None # Correct default for is_active
...
def reactivate_item(engine, item_id: int) -> bool:
    # ... (function unchanged) ...
    if not engine or item_id is None: return False
    reactivate_query = text("UPDATE items SET is_active = TRUE WHERE item_id = :id"); params = {"id": item_id}
    try:
        with engine.connect() as connection:
...
def update_supplier(engine, supplier_id: int, updated_details: Dict[str, Any]) -> bool:
    ...
    for key, value in updated_details.items():
        # Ensure value is appropriate before adding (basic check)
        if key in editable_fields:
             update_parts.append(f"{key} = :{key}")
             # Use None if value is empty string, otherwise pass the value
...
if not db_engine:
    st.info("DB connection failed. Check secrets.")
    st.stop()
else:
    ...
    st.divider()
    st.divider() # Extra divider for visual separation
        ...
        with st.form("new_supplier_form", clear_on_submit=True):
            ...
            if s_submitted:
                if not new_s_name: st.warning("Supplier Name is required.")
                    ...
                    supplier_data = {
                        "name": new_s_name.strip(), "contact_person": new_s_contact.strip() or None,
                        "phone": new_s_phone.strip() or None, "email": new_s_email.strip() or None,
                        "address": new_s_address.strip() or None, "notes": new_s_notes.strip() or None
                    }
    ...
    def load_supplier_for_edit(): # Callback for supplier selectbox
        selected_option = st.session_state.supplier_to_edit_select
        supplier_id_to_load = selected_option[1] if selected_option else None
        if supplier_id_to_load:
            details = get_supplier_details(db_engine, supplier_id_to_load)
            st.session_state.supplier_to_edit_id = supplier_id_to_load if details else None
            st.session_state.edit_supplier_form_values = details if details else None
    ...
    selected_supplier_tuple = st.selectbox(
            ...
            with st.form("edit_supplier_form"):
                ...
                if s_update_submitted:
                    if not edit_s_name: st.warning("Supplier Name cannot be empty.")
                        ...
                        # *** FIXED: Use safe stripping for supplier details ***
                        s_updated_data = {
                            "name": st.session_state.edit_s_name.strip() if st.session_state.edit_s_name else "",
                        ...
                        # Basic validation after stripping
                        if not s_updated_data["name"]:
                            st.warning("Supplier Name cannot be empty after stripping.")
                        else:
                            s_update_success = update_supplier(db_engine, st.session_state.supplier_to_edit_id, s_updated_data)
                            if s_update_success:
                                st.success(f"Supplier '{s_updated_data['name']}' updated!")
                                get_all_suppliers.clear() # Clear supplier cache
                                st.session_state.supplier_to_edit_id = None
                                st.session_state.edit_supplier_form_values = None
                                st.rerun()
            st.divider()
            st.subheader("Deactivate Supplier"); st.warning("⚠️ Deactivating removes supplier from active lists.")
    ...
    with col_mov1: # Receiving Column
    ...
    with col_mov2: # Adjustment Column
        with st.expander("📈 Record Stock Adjustment", expanded=False):
            # ... (Adjustment form unchanged) ...
            adj_item_options = [("--- Select ---", None)] + item_options_list # Allow adjusting inactive
