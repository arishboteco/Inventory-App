...
if not db_engine:
    ...
    with tab_view:
        ...
        # Fetch data once based on checkbox state
        items_df_with_stock = get_all_items_with_stock(db_engine, include_inactive=st.session_state.show_inactive)

        # *** LOW STOCK REPORT SECTION HAS BEEN REMOVED FROM THIS PAGE ***
        # (It now lives on the main item_manager_app.py page)
    ...
    with tab_manage:
        ...
        def load_item_for_edit(): # Callback needs access to db_engine
        ...
        selected_item_tuple = st.selectbox("Select Item:", options=edit_options, format_func=lambda x: x[0], key="item_to_edit_select", on_change=load_item_for_edit, index=current_index, label_visibility="collapsed")
