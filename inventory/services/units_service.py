...
class UnitsService:
    """Service for managing unit conversions and display according to the units table architecture."""
    ...
    def get_unit_info(unit_id: int) -> Dict:
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT base_unit, purchase_unit, conversion_factor FROM units WHERE unit_id = %s",
                    [unit_id]
                )
                row = cursor.fetchone()
                if row:
    ...
    def get_all_units() -> List[Dict]:
        ...
        try:
            ...
            return [
                {'unit_id': 1, 'base_unit': 'GM', 'purchase_unit': '2 KG', 'conversion_factor': 2000.0},
                {'unit_id': 19, 'base_unit': 'GM', 'purchase_unit': 'KG', 'conversion_factor': 1000.0},
                {'unit_id': 55, 'base_unit': 'PC', 'purchase_unit': 'PC', 'conversion_factor': 1.0},
            ]
