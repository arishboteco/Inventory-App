...
def fetch_indent_page_data_pg5(_engine):  # Page-specific function
    ...
    items_df_pg5 = item_service.get_all_items_with_stock(
        _engine, include_inactive=False
    )
    required_cols_pg5 = [
        "item_id",
        "name",
    ...
    if not all(col in items_df_pg5.columns for col in required_cols_pg5):
        missing_pg5 = [
            col for col in required_cols_pg5 if col not in items_df_pg5.columns
        ]
        print(
    ...
    return items_df_pg5[
        required_cols_pg5
    ].copy(), item_service.get_distinct_departments_from_items(_engine)
...
def generate_indent_pdf(
    indent_header: Dict, indent_items: List[Dict]
) -> Optional[bytes]:
    ...
    try:
        ...
        pdf.cell(
            0, 4, generated_time_str, align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT
        )
        pdf.ln(2)
        ...
        for key_pdf, header_text_pdf in [
            ...
            align_pdf = (
                "C"
                if key_pdf in ["sno", "unit"]
                else ("R" if key_pdf == "qty" else "L")
            )
            pdf.cell(
                col_widths[key_pdf],
                table_header_height,
        ...
        if not indent_items:
            ...
            for item_pdf in indent_items:
                ...
                effective_row_height = (
                    max_lines * base_line_height_items + (max_lines - 1) * 0.5
                )
...
def add_suggested_item_callback_pg5(item_id_to_add, item_name_to_add, unit_to_add):
    if any(
        row.get("item_id") == item_id_to_add
    ...
    if empty_row_idx != -1:
        ...
        st.session_state.pg5_create_indent_rows.append(
            {"id": new_id_pg5, **new_row_data_pg5}
        )
        st.session_state.pg5_create_indent_next_id += 1
        st.toast(
            f"Added '{item_name_to_add}' as a new line.", icon="➕"
        )  # Corrected icon


def update_row_item_details_callback_pg5(
    row_index, item_selectbox_key_arg, item_options_dict_arg
):
...
if st.session_state.pg5_active_indent_section == "create":
    ...
    if st.session_state.get(PG5_CREATE_INDENT_HEADER_RESET_SIGNAL_SESS_KEY, False):
        ...
        st.session_state[PG5_CREATE_INDENT_DATE_REQ_SESS_KEY] = (
    ...
    if st.session_state.get("pg5_last_created_mrn_for_print"):
        ...
        if st.session_state.get("pg5_last_submitted_indent_details"):
            ...
            summary_items_pg5 = st.session_state.pg5_last_submitted_indent_details[
                "items"
            ]
        ...
        with whatsapp_col_pg5:
            if st.session_state.get("pg5_last_submitted_indent_details"):
                header_pg5 = st.session_state.pg5_last_submitted_indent_details[
                    "header"
                ]
                wa_text_pg5 = (
        ...
        with st.form("pg5_create_indent_final_submit_form", clear_on_submit=False):
            ...
            if submitted_final_button_pg5:
                ...
                current_item_options_for_validation_pg5 = (
                    item_options_dict_pg5
                    if "item_options_dict_pg5" in locals()
                    and isinstance(item_options_dict_pg5, dict)
                    else {}
                )  # Use correct dict

                for i_r_pg5, r_data_pg5 in enumerate(
                    st.session_state.pg5_create_indent_rows
                ):
                ...
                if is_valid_pg5:
                    ...
                    if new_mrn_pg5:
                        ...
                        if success_pg5:
                            ...
                            hd_summary_pg5, itms_summary_pg5 = (
                                indent_service.get_indent_details_for_pdf(
                                    db_engine, new_mrn_pg5
                                )
                            )
                            ...
                            st.session_state.pg5_create_indent_next_id = 1
                            st.session_state[
                                PG5_CREATE_INDENT_HEADER_RESET_SIGNAL_SESS_KEY
                            ] = True
...
# --- VIEW INDENTS SECTION ---
    ...
    date_start_str_arg_pg5 = (
        date_start_filter_val_pg5.strftime("%Y-%m-%d")
        if date_start_filter_val_pg5
        else None
    )
    date_end_str_arg_pg5 = (
        date_end_filter_val_pg5.strftime("%Y-%m-%d")
        if date_end_filter_val_pg5
        else None
    )
    dept_arg_pg5 = (
        dept_filter_val_pg5 if dept_filter_val_pg5 != FILTER_ALL_DEPARTMENTS else None
    )
    status_arg_pg5 = (
        status_filter_val_pg5 if status_filter_val_pg5 != FILTER_ALL_STATUSES else None
    )
        ...
        for col_v_pg5 in date_cols_for_view_pg5:
            if col_v_pg5 in display_df_pg5.columns:
                display_df_pg5[col_v_pg5] = (
                    pd.to_datetime(
                        display_df_pg5[col_v_pg5], errors="coerce"
                    ).dt.strftime("%Y-%m-%d %H:%M")
                    if col_v_pg5
                    in ["date_submitted", "created_at", "updated_at", "date_processed"]
                    else pd.to_datetime(
                        display_df_pg5[col_v_pg5], errors="coerce"
                    ).dt.strftime("%Y-%m-%d")
                )
            ...
            if st.session_state.get("pg5_pdf_filename_for_download"):
                try:
                    mrn_from_filename_pg5 = (
                        st.session_state.pg5_pdf_filename_for_download.split("_", 1)[
                            1
                        ].rsplit(".", 1)[0]
                    )
                    if mrn_from_filename_pg5 in available_mrns_for_pdf_pg5:
                        current_pdf_selection_idx_pg5 = (
                            available_mrns_for_pdf_pg5.index(mrn_from_filename_pg5)
                        )
                except IndexError:
                    pass
            ...
            if selected_mrn_for_pdf_pg5 != PLACEHOLDER_SELECT_MRN_PDF:
                if not (
                    ...
                    if st.button(
                        ...
                        with st.spinner(
                            f"Generating PDF for {selected_mrn_for_pdf_pg5}..."
                        ):
                            ...
                            if header_details_pg5 and items_details_pg5 is not None:
                                ...
                                if pdf_bytes_pg5:
                                    st.session_state.pg5_pdf_bytes_for_download = (
                                        pdf_bytes_pg5
                                    )
                                    st.session_state.pg5_pdf_filename_for_download = (
...
# --- PROCESS INDENT SECTION ---
    ...
    if selected_indent_id_pg5:
        ...
        if (
            ...
            and "indent_id_for_check"
            in st.session_state.pg5_process_indent_items_df.columns
            and not st.session_state.pg5_process_indent_items_df.empty
            and st.session_state.pg5_process_indent_items_df.iloc[0][
                "indent_id_for_check"
            ]
        ...
        if not items_df_for_processing_disp_pg5.empty:
            ...
            for col_header_pg5, header_txt_pg5 in zip(
                header_cols_proc_pg5, headers_text_list_pg5
            ):
                col_header_pg5.markdown(f"**{header_txt_pg5}**")

            with st.form(
                f"pg5_process_indent_form_submit_{selected_indent_id_pg5}",
                ...
                for (
                    ...
                    issue_qty_key_proc_pg5 = (
                        f"pg5_issue_qty_input_{indent_item_id_proc_pg5}"
                    )

                    item_display_cols_proc_pg5 = st.columns(
                        [3, 1, 1, 1, 1.5, 1.5, 1.5, 2]
                    )
                    # ... (display item details) ...
                    item_display_cols_proc_pg5[0].write(
                        f"{row_proc_item_disp_pg5['item_name']} ({row_proc_item_disp_pg5['item_unit']})"
                    )
                    item_display_cols_proc_pg5[1].write(
                        f"{row_proc_item_disp_pg5['requested_qty']:.2f}"
                    )
                    # ... (other display columns) ...
                    item_display_cols_proc_pg5[3].write(
                        f"{row_proc_item_disp_pg5['qty_remaining_to_issue']:.2f}"
                    )
                    item_display_cols_proc_pg5[4].write(
                        f"{row_proc_item_disp_pg5['stock_on_hand']:.2f}"
                    )
                    item_display_cols_proc_pg5[5].caption(
                    ...
                    stock_available_calc_pg5 = float(
                        row_proc_item_disp_pg5.get("stock_on_hand", 0.0)
                    )
                    max_issuable_qty_calc_pg5 = min(
                        qty_remaining_calc_pg5, stock_available_calc_pg5
                    )
                    ...
                    item_display_cols_proc_pg5[7].caption(
                        row_proc_item_disp_pg5.get("item_notes") or "---"
                    )
                    st.caption("")
                ...
                submit_issue_button_pg5 = st.form_submit_button(
                    "💾 Issue Quantities & Update Status",
                    type="primary",
                    use_container_width=True,
                )
                    ...
                    if not st.session_state.pg5_process_indent_user_id.strip():
                        ...
                        for (
                            ...
                            current_issue_qty_key_s_pg5 = (
                                f"pg5_issue_qty_input_{ii_id_s_pg5}"
                            )
                            qty_val_form_s_pg5 = st.session_state.get(
                                current_issue_qty_key_s_pg5, 0.0
                            )
                            try:
                        ...
                        if (
                            ...
                            with st.spinner(f"Processing indent {selected_mrn_pg5}..."):
                                ...
                                if success_p_pg5:
                                    ...
                                    st.session_state.pg5_process_indent_items_df = (
                                        pd.DataFrame()
                                    )
                                    st.session_state.pg5_process_indent_issue_quantities_defaults = (
                                        {}
