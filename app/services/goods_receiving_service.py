...
def generate_grn_number(engine: Engine) -> Optional[str]:
    ...
    """
    if engine is None: 
        print("ERROR [goods_receiving_service.generate_grn_number]: Database engine not available.")
        return None
    try:
        ...
        print(f"ERROR [goods_receiving_service.generate_grn_number]: Error generating GRN Number: {e}\n{traceback.format_exc()}")
        return None
...
def create_grn(engine: Engine, grn_data: Dict[str, Any], items_received_data: List[Dict[str, Any]]) -> Tuple[bool, str, Optional[int]]:
    ...
    missing_fields = [f for f in required_header if not grn_data.get(f) or (isinstance(grn_data.get(f), str) and not str(grn_data.get(f)).strip())]
    if missing_fields:
    ...
    for i, item in enumerate(items_received_data):
        try:
            ...
            return False, f"Invalid numeric data for quantity/price in received item line {i+1}.", None
    ...
    try:
        with engine.connect() as conn: 
            with conn.begin(): 
                ...
                if not new_grn_id: 
                    raise Exception("Failed to retrieve grn_id after GRN header insertion.")
...
def get_received_quantities_for_po(_engine: Engine, po_id: int) -> pd.DataFrame:
    ...
    if 'total_previously_received' not in df.columns:
        ...
        df['total_previously_received'] = pd.to_numeric(df['total_previously_received'], errors='coerce').fillna(0.0)
    return df

@st.cache_data(ttl=120, show_spinner="Fetching GRN list...")
def list_grns(_engine: Engine, filters: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
    """
    ...
    """
    if _engine is None: 
        print("ERROR [goods_receiving_service.list_grns]: Database engine not available.")
        return pd.DataFrame()
...
def get_grn_details(_engine: Engine, grn_id: int) -> Optional[Dict[str, Any]]:
    ...
    if _engine is None or not grn_id: 
        print("ERROR [goods_receiving_service.get_grn_details]: Database engine or GRN ID not provided.")
        return None
    ...
    try:
        ...
        if grn_header_df.empty: 
            print(f"WARNING [goods_receiving_service.get_grn_details]: No GRN header found for grn_id {grn_id}.")
            return None
        ...
        print(f"ERROR [goods_receiving_service.get_grn_details]: DB error fetching GRN details for grn_id {grn_id}: {e}\n{traceback.format_exc()}")
        return None