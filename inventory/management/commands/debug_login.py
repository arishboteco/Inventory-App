    ...
    def handle(self, *args, **options):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                self.stdout.write(self.style.SUCCESS(f'✅ Database connection: OK (result: {result})'))
        ...
        self.stdout.write(f'   Using password from env: {"*" * len(env_password)}')
        
        try:
            authenticated_user = authenticate(username='admin', password=env_password)
            if authenticated_user:
                self.stdout.write(self.style.SUCCESS(f'✅ Authentication successful: {authenticated_user.username}'))
            else:
                self.stdout.write(self.style.ERROR('❌ Authentication failed'))
                
                # Try with the hardcoded password
                self.stdout.write('\n🔄 Trying with fallback password...')
                fallback_authenticated = authenticate(username='admin', password='admin123!')
                if fallback_authenticated:
                    self.stdout.write(self.style.SUCCESS('✅ Fallback authentication successful'))
                else:
                    self.stdout.write(self.style.ERROR('❌ Fallback authentication also failed'))
        except Exception as e:
        ...
        try:
            ...
            self.stdout.write(f'   Hash algorithm: {admin_user.password.split("$")[0] if "$" in admin_user.password else "Unknown"}')
            
            # Test if password needs to be reset
            if admin_user.check_password(env_password):
                self.stdout.write(self.style.SUCCESS('✅ Password hash matches environment password'))
            elif admin_user.check_password('admin123!'):
                self.stdout.write(self.style.WARNING('⚠️  Password hash matches fallback, but not environment'))
            else:
                self.stdout.write(self.style.ERROR('❌ Password hash matches neither password'))
        except Exception as e:
        ...
        for var in important_vars:
            ...
            if var in ['DJANGO_SECRET_KEY', 'DATABASE_URL', 'DJANGO_SUPERUSER_PASSWORD']:
                display_value = f"{'*' * min(10, len(value))}..." if value != 'NOT SET' else 'NOT SET'
            else:
                display_value = value
