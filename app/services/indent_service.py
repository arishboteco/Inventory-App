...
from app.core.constants import (
    ...
    STATUS_PROCESSING,
    STATUS_COMPLETED, 
    STATUS_CANCELLED, # Added for cancel_entire_indent
    ITEM_STATUS_PENDING_ISSUE,
    ITEM_STATUS_FULLY_ISSUED,
    ITEM_STATUS_PARTIALLY_ISSUED,
    ITEM_STATUS_CANCELLED, # Renamed from ITEM_STATUS_CANCELLED_ITEM for consistency
    TX_INDENT_FULFILL 
)
...
def create_indent(engine, indent_data: Dict[str, Any], items_data: List[Dict[str, Any]]) -> Tuple[bool, str]:
    ...
    for i, item in enumerate(items_data):
        if not item.get('item_id') or not item.get('requested_qty') or float(item['requested_qty']) <= 0:
    ...
    try:
        ...
        st.error(f"{error_msg} Details: {e}")
...
def get_indents_for_processing(_engine) -> pd.DataFrame:
    ...
    query_str = """
        SELECT 
            ...
            i.date_required,
            i.status
            -- Removed item_count from here as it might not be needed for selection,
            -- and reduces group by complexity if we add more direct indent fields.
            -- If item_count is desired, it needs to be in GROUP BY or an aggregate.
        FROM indents i
...
def get_indent_items_for_display(_engine, indent_id: int) -> pd.DataFrame:
    if _engine is None or not indent_id:
        st.error("Database engine not available or invalid indent ID.")
        return pd.DataFrame()
    ...
    if not df.empty:
        ...
        df['qty_remaining_to_issue'] = df['requested_qty'] - df['issued_qty']
        df.loc[df['qty_remaining_to_issue'] < 0, 'qty_remaining_to_issue'] = 0
...
def get_indent_details_for_pdf(engine, mrn: str) -> Tuple[Optional[Dict[str, Any]], Optional[List[Dict[str, Any]]]]:
    if engine is None or not mrn:
        st.error("Database engine or MRN not provided for PDF details.")
        return None, None
...
def process_indent_issuance(_engine, indent_id: int, items_to_issue: List[Dict], user_id: str, indent_mrn: str) -> Tuple[bool, str]:
    ...
    try:
            ...
            with connection.begin(): 
                for item_data in items_to_issue:
                    ...
                    if qty_still_pending <= 0:
                        processed_messages.append(f"Item ID {item_id}: Already fulfilled. No further issuance.")
                        continue
                    ...
                    if qty_to_issue_now > qty_still_pending:
                        ...
                        processed_messages.append(f"Item ID {item_id}: Issue qty ({original_qty_to_issue_now_for_msg:.2f}) reduced to pending ({qty_to_issue_now:.2f}).")
                    ...
                    if qty_to_issue_now <= 0:
                        processed_messages.append(f"Item ID {item_id}: Skipped (no stock or qty became zero after clamping).")
                        continue
                    
                    update_item_stock_sql = text("UPDATE items SET current_stock = current_stock - :qty WHERE item_id = :item_id;")
                    connection.execute(update_item_stock_sql, {"qty": qty_to_issue_now, "item_id": item_id})

                    insert_stock_tx_sql = text("""
                        INSERT INTO stock_transactions 
                        (item_id, quantity_change, transaction_type, user_id, related_mrn, transaction_date)
                        VALUES (:item_id, :qty_change, :tx_type, :user_id, :mrn, NOW());
                    """)
                    connection.execute(insert_stock_tx_sql, {
                        "item_id": item_id, 
                        "qty_change": -qty_to_issue_now,
                        "tx_type": TX_INDENT_FULFILL,
                        "user_id": user_id, 
                        "mrn": indent_mrn
                    })

                    new_total_issued_for_item = current_issued_qty + qty_to_issue_now
                    new_item_status = ITEM_STATUS_PARTIALLY_ISSUED
                    if new_total_issued_for_item >= current_requested_qty:
                        new_item_status = ITEM_STATUS_FULLY_ISSUED
                    ...
                    processed_messages.append(f"Item ID {item_id}: Issued {qty_to_issue_now:.2f}. Status: {new_item_status}.")
                ...
                all_statuses = [row[0] for row in item_statuses_results] if item_statuses_results else []
                overall_indent_status = STATUS_PROCESSING 

                if not all_statuses: 
                    overall_indent_status = STATUS_SUBMITTED 
                elif all(s == ITEM_STATUS_FULLY_ISSUED or s == ITEM_STATUS_CANCELLED for s in all_statuses):
                    overall_indent_status = STATUS_COMPLETED
                elif not any(s == ITEM_STATUS_PENDING_ISSUE or s == ITEM_STATUS_PARTIALLY_ISSUED for s in all_statuses):
                    overall_indent_status = STATUS_COMPLETED
                
                if any_actual_issuance_occurred or overall_indent_status == STATUS_COMPLETED:
                    _update_indent_overall_status(connection, indent_id, overall_indent_status, user_id)
                    processed_messages.append(f"Indent MRN {indent_mrn} status updated to: {overall_indent_status}.")
                elif not any_actual_issuance_occurred and not items_to_issue: 
                     processed_messages.append(f"No items specified for issuance in MRN {indent_mrn}.")
                else: 
                    processed_messages.append(f"No actual stock was issued for MRN {indent_mrn} in this batch.")
        ...
        st.error(f"Error during indent processing for MRN {indent_mrn}: {e}")
        print(traceback.format_exc())
        detailed_error_message = f"Error: {e}."
        if processed_messages:
...
def mark_indent_completed(_engine, indent_id: int, user_id: str, indent_mrn: str) -> Tuple[bool, str]:
    """
    Manually marks an indent as completed.
    Args:
        _engine: SQLAlchemy database engine.
        indent_id: ID of the indent.
        user_id: ID/Name of the user marking as completed.
        indent_mrn: MRN of the indent for messaging.
    Returns:
        Tuple (bool, str) indicating success and a message.
    """
    if _engine is None: return False, "Database engine not available."
    if not all([indent_id, user_id, indent_mrn]):
        return False, "Missing indent_id, user_id, or indent_mrn."
    
    try:
        with _engine.connect() as connection:
            with connection.begin():
                # Optional: Update any PENDING items to a status like 'NOT_ISSUED' or leave as is.
                # For now, we just update the main indent.
                # update_pending_items_sql = text("""
                #     UPDATE indent_items 
                #     SET item_status = :new_status 
                #     WHERE indent_id = :indent_id AND item_status = :pending_status;
                # """)
                # connection.execute(update_pending_items_sql, {
                #     "new_status": ITEM_STATUS_CANCELLED, # Or a new 'NOT_ISSUED' status
                #     "indent_id": indent_id,
        ...
        get_indents.clear()
        get_indents_for_processing.clear()
        return True, f"Indent MRN {indent_mrn} successfully marked as {STATUS_COMPLETED}."
    except Exception as e:
        st.error(f"Error marking indent {indent_mrn} as completed: {e}")
        print(traceback.format_exc())
...
def cancel_entire_indent(_engine, indent_id: int, user_id: str, indent_mrn: str) -> Tuple[bool, str]:
    """
    Cancels an entire indent and all its items.
    Args:
        _engine: SQLAlchemy database engine.
        indent_id: ID of the indent.
        user_id: ID/Name of the user cancelling the indent.
        indent_mrn: MRN of the indent for messaging.
    Returns:
        Tuple (bool, str) indicating success and a message.
    """
    ...
    try:
        with _engine.connect() as connection:
            with connection.begin():
                # Update all items in the indent to 'Cancelled'
                # Only update if they are not already fully issued (though cancelling a fully issued item is unusual)
                update_items_sql = text("""
                    UPDATE indent_items 
                    SET item_status = :cancelled_status 
                    WHERE indent_id = :indent_id AND item_status != :fully_issued_status; 
                """)
                # Consider if you want to allow cancelling fully issued items.
                # If so, remove "AND item_status != :fully_issued_status"
                # However, cancelling items that have already impacted stock would be complex (require reversal).
                # For now, we assume cancellation applies to non-issued or partially issued items.
                connection.execute(update_items_sql, {
                    "cancelled_status": ITEM_STATUS_CANCELLED,
                    "indent_id": indent_id,
                    "fully_issued_status": ITEM_STATUS_FULLY_ISSUED 
                })
                
                _update_indent_overall_status(connection, indent_id, STATUS_CANCELLED, user_id)
        
        get_indents.clear()
        get_indents_for_processing.clear()
        return True, f"Indent MRN {indent_mrn} and its pending items successfully cancelled."
    except Exception as e:
        st.error(f"Error cancelling indent {indent_mrn}: {e}")
        print(traceback.format_exc())
