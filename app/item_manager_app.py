...
# ─────────────────────────────────────────────────────────
def run_dashboard():
    st.set_page_config(page_title="Restaurant Inventory Manager", page_icon="🍲", layout="wide")
    st.title("🍲 Restaurant Inventory Dashboard")
    st.caption(f"Current Overview as of: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    st.write("Welcome to your central hub for managing restaurant inventory.") # Added intro line
    st.divider()
    ...
    kpi_cols[0].metric("📦 Active Items", total_active_items, help="Total number of unique items currently marked as active.")
    kpi_cols[1].metric("📉 Low Stock Items", low_stock_count, help="Items at or below their defined reorder point (and reorder point > 0).")
    kpi_cols[2].metric("🚚 Active Suppliers", total_active_suppliers, help="Total number of suppliers currently marked as active.")
    if pending_indents_count is not None: # Check if we fetched this data
        kpi_cols[3].metric("📝 Pending Indents", pending_indents_count, help="Material requests awaiting processing.")
