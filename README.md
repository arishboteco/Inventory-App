...
Refer to [STYLEGUIDE.md](STYLEGUIDE.md) for design tokens, reusable components and usage examples. When creating new templates or JavaScript-driven components, prefer Tailwind CSS utilities and the classes defined in `static/css/app.css` before adding custom styles. Semantic color utilities like `bg-success` and `text-warning` are available and should be used instead of hard-coded palette classes.
