(function(){
  ...
  const trigger = document.createElement('input');
    ...
    trigger.className = (sel.className || '').replace(/\bpredictive\b/, '').trim();
    const placeholder = sel.getAttribute('data-placeholder') || sel.getAttribute('placeholder') || 'Filter options...';
    trigger.placeholder = placeholder;

  const dropdown = document.createElement('div');
  dropdown.className = 'absolute z-[60] mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto w-full hidden';

    // Build option list snapshot
  let options = Array.from(sel.options).map(o=>({value:o.value, label:o.text, disabled:o.disabled, selected:o.selected}));
    ...
    function renderChips(){
      ...
      selected.forEach(o=>{
        ...
        pill.className = 'inline-flex items-center gap-1 bg-blue-50 text-blue-800 border border-blue-200 rounded-full px-2 py-0.5 text-xs';
        ...
        btn.addEventListener('click', ()=>{
          o.selected = false;
          const opt = Array.from(sel.options).find(so=>so.value===o.value);
          if (opt) opt.selected = false;
          renderChips();
          trigger.value = '';
          trigger.setAttribute('aria-label', 'Filter options');
          dispatchChange();
          if (!dropdown.classList.contains('hidden')) rebuildList(trigger.value);
        });
    ...
    function syncTrigger(){
      ...
      trigger.value = '';
      trigger.setAttribute('aria-label', summary ? `Selected: ${summary}` : 'Filter options');
      renderChips();
    }
    ...
    function rebuildList(filter){
      ...
      toolbar.className = 'sticky top-0 bg-white p-2 border-b border-gray-200 flex items-center gap-2';
      const btnSelectAll = document.createElement('button');
      btnSelectAll.type = 'button';
      btnSelectAll.className = 'text-xs px-2 py-1 rounded border border-gray-300 hover:bg-gray-50';
      btnSelectAll.textContent = 'Select all';
      const btnClearAll = document.createElement('button');
      btnClearAll.type = 'button';
      btnClearAll.className = 'text-xs px-2 py-1 rounded border border-gray-300 hover:bg-gray-50';
      ...
      const visible = options
        .filter(o=>o.value && !o.disabled && (q ? (String(o.label).toLowerCase().includes(q)) : true));
      visible
        .forEach(o=>{
      ...
      btnClearAll.addEventListener('click', ()=>{
        ...
        Array.from(sel.options).forEach(so=>{ if (so.value) so.selected = false; });
    ...
    function refreshOptions(){
      options = Array.from(sel.options).map(o=>({value:o.value, label:o.text, disabled:o.disabled, selected:o.selected}));
      syncTrigger();
    ...
    trigger.addEventListener('keydown', (e)=>{
      ...
      if (e.key === 'Backspace' && (trigger.value || '').length === 0){
        ...
        if (last){
          last.selected = false;
          const opt = Array.from(sel.options).find(so=>so.value===last.value);
          if (opt) opt.selected = false;
          syncTrigger();
          dispatchChange();
          rebuildList('');
    ...
    const mo = new MutationObserver((mutations)=>{
      for (const m of mutations){
        if (m.type === 'childList') { refreshOptions(); break; }
      }
    });
  ...
  function init(root){
    ...
    scope.querySelectorAll('select.predictive[multiple]').forEach(enhanceMultiSelect);
  }
