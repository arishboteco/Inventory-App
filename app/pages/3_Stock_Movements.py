...
try:
    ...
    from app.core.constants import (
        TX_RECEIVING, TX_ADJUSTMENT, TX_WASTAGE,
...
for section_key_pg3 in SECTION_KEYS_PG3: 
    ...
    if section_key_pg3 == "receive" and "pg3_receive_po_id_form_val" not in st.session_state: 
...
item_options_with_placeholder_list_pg3 = [placeholder_option_stock_select_pg3] + active_item_options_list_pg3
...
def update_selected_item_id_callback_pg3(tuple_key_arg_pg3, id_key_arg_pg3): 
    ...
    st.session_state[id_key_arg_pg3] = selected_tuple_val_pg3[1] if selected_tuple_val_pg3 and selected_tuple_val_pg3[1] != -1 else None
...
current_selected_item_id_key_pg3 = f"pg3_{active_section_prefix_val_pg3}_selected_item_id"
...
if st.session_state.get(current_reset_signal_key_pg3, False):
    ...
    st.session_state[current_qty_ss_key_pg3] = 0.0 if active_section_prefix_val_pg3 == "adjust" else 0.01 
...
if active_section_prefix_val_pg3 == "adjust": st.caption("Correct discrepancies or make other non-wastage adjustments.")
elif active_section_prefix_val_pg3 == "waste": st.caption("Record items damaged, expired, or otherwise unusable.")

item_col_main_pg3, _ = st.columns([2,1]) 
with item_col_main_pg3:
    default_item_index_val_pg3 = 0 
    current_selected_item_id_val_pg3 = st.session_state.get(current_selected_item_id_key_pg3)
    if current_selected_item_id_val_pg3 is not None:
        try: 
            default_item_index_val_pg3 = [opt[1] for opt in item_options_with_placeholder_list_pg3].index(current_selected_item_id_val_pg3)
        except ValueError: 
            default_item_index_val_pg3 = 0 
            st.session_state[current_selected_item_id_key_pg3] = None 
            st.session_state[current_item_tuple_key_pg3] = placeholder_option_stock_select_pg3
    ...
    st.selectbox(
        "Item*", options=item_options_with_placeholder_list_pg3, format_func=lambda x_opt: x_opt[0],
    ...
    selected_item_id_for_stock_disp_pg3 = st.session_state.get(current_selected_item_id_key_pg3)
    if selected_item_id_for_stock_disp_pg3 and selected_item_id_for_stock_disp_pg3 != -1:
        item_details_stock_disp_pg3 = item_service.get_item_details(db_engine, selected_item_id_for_stock_disp_pg3)
        if item_details_stock_disp_pg3: 
            st.caption(f"Current Stock: {item_details_stock_disp_pg3.get('current_stock', 0):.2f} {item_details_stock_disp_pg3.get('unit', '')}")
        else: 
...
with st.form(form_key_main_pg3, clear_on_submit=False): 
    ...
    with qty_form_col_main_pg3:
        ...
        PRACTICAL_MAX_NEGATIVE_NUMBER_PG3 = -1.7976931348623157e+308 
        min_widget_val_form_pg3 = 0.01 
        help_text_qty_form_pg3 = "Enter the quantity for this movement."

        if active_section_prefix_val_pg3 == "receive":
            qty_label_form_pg3 = "Quantity Received*"; help_text_qty_form_pg3 = "Quantity of goods received."
            ...
            help_text_qty_form_pg3 = "Positive value for stock increase, negative value for decrease."
        elif active_section_prefix_val_pg3 == "waste":
            qty_label_form_pg3 = "Quantity Wasted*"; help_text_qty_form_pg3 = "Quantity of items wasted (will be deducted from stock)."
        
        # MODIFIED: Default initial_qty_value for number_input
        default_qty_for_section = 0.0 if active_section_prefix_val_pg3 == "adjust" else min_widget_val_form_pg3 # Default to min_value for receive/waste
        initial_qty_value_form_pg3 = float(st.session_state.get(current_qty_ss_key_pg3, default_qty_for_section))
        # Ensure initial value is not below min_value if session state had an invalid value (e.g. 0 for receive/waste)
        if active_section_prefix_val_pg3 != "adjust" and initial_qty_value_form_pg3 < min_widget_val_form_pg3:
    ...
    with user_form_col_main_pg3:
        user_label_form_pg3 = "Recorder's Name/ID*" if active_section_prefix_val_pg3 != "receive" else "Receiver's Name/ID*"
    ...
    notes_label_form_pg3 = "Reason/Notes*" if active_section_prefix_val_pg3 in ["adjust","waste"] else "Notes / Remarks (Optional)"
    notes_placeholder_form_pg3 = "Crucial: Explain reason for adjustment/wastage..." if active_section_prefix_val_pg3 in ["adjust","waste"] else "e.g., Supplier name, Invoice #, Batch #..."
    ...
    if submitted_form_pg3: 
        selected_item_id_submit_pg3 = st.session_state.get(current_selected_item_id_key_pg3)
        qty_to_process_submit_pg3 = st.session_state.get(current_qty_ss_key_pg3, 0.0)
        user_id_to_process_submit_pg3 = st.session_state.get(current_user_id_ss_key_pg3, "")
        ...
        if not selected_item_id_submit_pg3 or selected_item_id_submit_pg3 == -1: 
            st.warning("⚠️ Please select an item first."); is_valid_submission_pg3 = False
        
        try:
            qty_float_submit_pg3 = float(qty_to_process_submit_pg3)
            if active_section_prefix_val_pg3 == "adjust" and qty_float_submit_pg3 == 0: 
                st.warning("⚠️ For 'Stock Adjustment', the quantity adjusted cannot be zero."); is_valid_submission_pg3 = False
            elif active_section_prefix_val_pg3 != "adjust" and qty_float_submit_pg3 <= 0: 
                st.warning(f"⚠️ Quantity for '{SECTIONS_PG3[active_section_prefix_val_pg3]}' must be greater than 0."); is_valid_submission_pg3 = False
        except (ValueError, TypeError):
            st.warning("⚠️ Invalid quantity entered. Please enter a valid number."); is_valid_submission_pg3 = False
        
        if not user_id_to_process_submit_pg3.strip():
            st.warning(f"⚠️ {user_label_form_pg3.replace('*','').strip()} is required."); is_valid_submission_pg3 = False
        if active_section_prefix_val_pg3 in ["adjust","waste"] and not notes_to_process_submit_pg3.strip():
            st.warning(f"⚠️ A reason/note is mandatory for '{SECTIONS_PG3[active_section_prefix_val_pg3]}'."); is_valid_submission_pg3 = False
        
        related_po_id_val_submit_pg3 = None
        if active_section_prefix_val_pg3 == "receive" and st.session_state.get(current_po_id_ss_key_pg3, "").strip():
            try:
                related_po_id_val_submit_pg3 = int(st.session_state.get(current_po_id_ss_key_pg3).strip())
            except ValueError:
                st.warning("⚠️ Related PO ID must be a numeric value if provided (enter only the number part)."); is_valid_submission_pg3 = False
        ...
        if is_valid_submission_pg3:
            ...
            if success_stock_tx_pg3:
                item_display_name_success_pg3_tuple = st.session_state.get(current_item_tuple_key_pg3)
                item_display_name_success_pg3 = item_display_name_success_pg3_tuple[0] if item_display_name_success_pg3_tuple else "Selected Item"
                st.success(f"✅ Successfully recorded {active_section_prefix_val_pg3} for '{item_display_name_success_pg3}'.")
                ...
                st.error(f"❌ Failed to record stock {active_section_prefix_val_pg3}. Check console for specific database errors.")