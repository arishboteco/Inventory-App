(function () {
  function resolveItemIdFromInput(inp) {
    ...
    if (list) {
      ...
      const match = opts.find(
        (o) =>
          (o.value || "").trim().toLowerCase() === typed.toLowerCase() ||
          (o.textContent || "").trim().toLowerCase() === typed.toLowerCase(),
      );
  ...
  function bindItemInputs(root) {
    ...
    scope
      .querySelectorAll("input[name^='items-'][name$='-item']")
      .forEach((itemInput) => {
        if (itemInput._indentBound) return;
        itemInput._indentBound = true;
        ...
        function refresh() {
          ...
          if (!id) {
            setInvalid("Choose a valid item from the list.");
            return;
          }
          fetch(`/items/meta/${id}/`)
            ...
            .then((data) => {
              if (!data || !data.ok) {
                setInvalid("Selected item not found.");
                return;
              }
              const parts = [];
  ...
  function checkDuplicates(root) {
    ...
    const form =
      scope && scope.querySelector
        ? scope.querySelector("#indent-form")
        : document.getElementById("indent-form");
    const submitBtn = form ? form.querySelector('button[type="submit"]') : null;
    const inputs = form
      ? Array.from(
          form.querySelectorAll("input[name^='items-'][name$='-item']"),
        )
      : [];
    const ids = inputs
      .map((inp) => resolveItemIdFromInput(inp))
      .filter(Boolean);
    const counts = ids.reduce((a, id) => {
      a[id] = (a[id] || 0) + 1;
      return a;
    }, {});
    let hasDup = false;
    inputs.forEach((inp) => {
      ...
      if (!warn) {
        ...
        warn.className = "mt-1 text-xs text-orange-600 hidden";
        warn.setAttribute("data-dup-warning", "");
        const wrap = inp.closest("div") || inp.parentElement;
        if (wrap) wrap.appendChild(warn);
      }
      if (id && counts[id] > 1) {
        hasDup = true;
        warn.textContent = "Duplicate item selected";
  ...
  function ensureHiddenIdsOnSubmit(root) {
    ...
    const form =
      scope && scope.querySelector
        ? scope.querySelector("#indent-form")
        : document.getElementById("indent-form");
    if (!form || form._boundHidden) return;
    form._boundHidden = true;
    form.addEventListener("submit", function (ev) {
      const inputs = form.querySelectorAll(
        "input[name^='items-'][name$='-item']",
      );
      let invalid = false;
      inputs.forEach((itemInput) => {
        ...
        if (!id) {
          invalid = true;
          if (err) {
            err.textContent = "Choose a valid item from the list.";
            err.classList.remove("hidden");
          }
          return;
        }
        let hidden = form.querySelector(
          `input[type="hidden"][data-item-hidden-for='${itemInput.name}']`,
        );
        if (!hidden) {
          hidden = document.createElement("input");
          hidden.type = "hidden";
          hidden.setAttribute("data-item-hidden-for", itemInput.name);
          form.appendChild(hidden);
        }
        hidden.name = itemInput.name;
        hidden.value = id;
        if (!itemInput.dataset.originalName)
          itemInput.dataset.originalName = itemInput.name;
        itemInput.name = itemInput.dataset.originalName + "_display";
        setTimeout(() => {
          itemInput.name = itemInput.dataset.originalName;
        }, 0);
      });
  ...
  function setupMultiAdd(root) {
    ...
    const addOneBtn =
      scope && scope.querySelector
        ? scope.querySelector("#add-row")
        : document.getElementById("add-row");
    const addRowsBtn =
      scope && scope.querySelector
        ? scope.querySelector("#add-rows-btn")
        : document.getElementById("add-rows-btn");
    const addRowsCount =
      scope && scope.querySelector
        ? scope.querySelector("#add-rows-count")
        : document.getElementById("add-rows-count");
    ...
    function clickAdd(times) {
      ...
      setTimeout(() => {
        const table =
          scope && scope.querySelector
            ? scope.querySelector("#items-table")
  ...
  function setupFormset(root) {
    ...
    const hasBits =
      scope && scope.querySelector
        ? !!scope.querySelector("#add-row") &&
          !!scope.querySelector("#items-table tbody")
        : !!document.getElementById("add-row");
    if (!hasBits) return;
  ...
  function setupDepartmentSync(root) {
    ...
    const ui =
      scope && scope.querySelector
        ? scope.querySelector("#department-ui")
        : document.getElementById("department-ui");
    const hidden =
      scope && scope.querySelector
        ? scope.querySelector("#id_department")
        : document.getElementById("id_department");
    if (hidden && ui) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (
      ...
      (!document.getElementById("modal-root") ||
        !document.getElementById("modal-root").contains(indentForm))
    ) {
