...
item_filter_options_pg4, transaction_type_filter_options_map_pg4 = fetch_filter_options_pg4(db_engine)
...
with filter_col1_pg4:
    ...
    try:
        ...
        item_filter_idx_pg4 = item_ids_in_options.index(current_selected_item_id_for_idx)
    except ValueError: 
    ...
    if current_trans_type_key_display_name in trans_type_options_keys_pg4:
        trans_type_idx_pg4 = trans_type_options_keys_pg4.index(current_trans_type_key_display_name)
    else: 
        st.session_state.pg4_filter_trans_type_key = FILTER_ALL_TYPES # Reset if invalid
...
filter_item_id_arg_pg4 = st.session_state.pg4_filter_item_id if st.session_state.pg4_filter_item_id != -1 else None
filter_trans_type_arg_pg4 = transaction_type_filter_options_map_pg4.get(st.session_state.pg4_filter_trans_type_key) 
filter_user_id_arg_pg4 = st.session_state.pg4_filter_user_id.strip() if st.session_state.pg4_filter_user_id else None
filter_related_mrn_arg_pg4 = st.session_state.pg4_filter_related_mrn.strip() if st.session_state.pg4_filter_related_mrn else None
...
if st.session_state.pg4_start_date_val <= st.session_state.pg4_end_date_val:
    ...
    if transactions_df_pg4.empty:
        ...
        if 'transaction_date' in display_df_pg4.columns:
            display_df_pg4['transaction_date_display'] = pd.to_datetime(display_df_pg4['transaction_date']).dt.strftime('%Y-%m-%d %H:%M:%S')
        else:
        ...
        if 'quantity_change' in display_df_pg4.columns:
            display_df_pg4['qty_display'] = format_qty_with_emoji_pg4(display_df_pg4['quantity_change'])
        else:
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "transaction_id": None, "item_id": None, "transaction_date": None, 
                "quantity_change": None, 
                "transaction_date_display": st.column_config.TextColumn("Timestamp", width="medium", help="Date and time of the transaction."),
                "item_name": st.column_config.TextColumn("Item Name", width="large"),
                "item_unit": st.column_config.TextColumn("Unit", width="small"),
                "transaction_type": st.column_config.TextColumn("Type", width="medium", help="Type of stock movement."),
                "qty_display": st.column_config.TextColumn("Qty Change", width="small", help="Stock change. ▲ In, ▼ Out."),
                "user_id": st.column_config.TextColumn("User/System ID", width="medium"), 
                ...
                "related_po_id": st.column_config.TextColumn("Related PO ID", width="medium"), 
            },
            column_order=[ 
                "transaction_date_display", "item_name", "item_unit", "transaction_type", "qty_display",
        ...
        def convert_df_to_csv_pg4(df_to_convert): 
            cols_to_export = [col for col in [
                "transaction_date", "item_name", "item_unit", "transaction_type", "quantity_change",
                "user_id", "notes", "related_mrn", "related_po_id" 
        ...
        st.download_button(
            label="📥 Download Report as CSV", data=csv_data_pg4,
            file_name=f"stock_history_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv", key="download_stock_history_csv_pg4_v5" 
        )
else: 
