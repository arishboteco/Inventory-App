...
def create_po(engine: Engine, po_data: Dict[str, Any], items_data: List[Dict[str, Any]]) -> Tuple[bool, str, Optional[int]]:
    ...
    header_query_obj = text("""INSERT INTO purchase_orders 
                             ...
                             RETURNING po_id;""")
    items_query_obj = text("""INSERT INTO purchase_order_items 
...
def update_po_status(engine: Engine, po_id: int, new_status: str, user_id: str) -> Tuple[bool, str]:
    ...
    try:
        with engine.connect() as conn:
            with conn.begin(): result = conn.execute(query_obj, params)
        if result.rowcount > 0:
            ...
            if current_db_status == new_status: 
                return True, f"PO ID {po_id} status is already '{new_status}'. No change made." 
