# pages/1_Items.py – full file with import-path fix and updated help text
...
def fetch_items_for_display(engine, show_inactive: bool) -> pd.DataFrame:
    """Cached fetch using the main app's function."""
    return get_all_items_with_stock(engine, include_inactive=show_inactive)
    ...
    with st.form("add_item_form", clear_on_submit=True):
        ...
        if submitted:
            ...
            if is_valid:
                ...
                if success:
                    st.success(message)
                    # No need to clear cache here as add_new_item does it
